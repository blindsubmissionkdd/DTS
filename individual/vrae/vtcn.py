...
class VRAE(BaseEstimator, nn.Module):
    ...
    def compute_loss(self, X):
        ...
        loss = recon_loss+mi_loss+tc_loss+dw_kl_loss
        return loss, recon_loss, dw_kl_loss, x
    ...
    def _batch_transform_distribution(self, x):
        """
        Passes the given input tensor into encoder and lambda function
        ...
        if self.is_fitted:
            with torch.no_grad():
                ...
                for t, x in enumerate(test_loader):
                    x = x[0]
                    x = x.permute(1, 0, 2)
                    samples_zCx_each, params_zCx_each = self._batch_transform_distribution(x)
                    samples_zCx.append(samples_zCx_each)
                    params_zCx.append(params_zCx_each)

                samples_zCx = np.concatenate(samples_zCx, axis=0)
                params_zCx = np.concatenate(params_zCx, axis=0)
                if save:
                    if os.path.exists(self.dload):
                        pass
                    else:
