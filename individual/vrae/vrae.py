...
class VRAE(BaseEstimator, nn.Module):
    ...
    def _rec2(self, x_decoded, x, latent_sample, latent_dist, n_data, is_mss, loss_fn):
        recon_loss = loss_fn(x_decoded, x)

        log_pz, log_qz, log_prod_qzi, log_q_zCx = get_log_pz_qz_prodzi_qzCx(latent_sample,
                                                                             latent_dist,
                                                                             n_data,
                                                                             is_mss=is_mss)
        ...
        # total loss
        return recon_loss, mi_loss, tc_loss, dw_kl_loss*anneal_reg
    ...
    def _rec(self, x_decoded, x, loss_fn):
        ...
        # q_z = Normal(latent_mean, (0.5 * latent_logvar).exp())
    ...
    def compute_loss(self, X):
        ...
        latent_mean = self.lmbd.hidden_to_mean(cell_output)
        latent_logvar = self.lmbd.hidden_to_logvar(cell_output)

        if self.training:
            std = torch.exp(0.5 * latent_logvar)
            eps = torch.randn_like(std)
            latent_sample= eps.mul(std).add_(latent_mean)
        else:
            latent_sample=latent_mean

        latent_dist = (latent_mean, latent_logvar)


        # latent = self.lmbd(cell_output)
        x_decoded = self.decoder(latent_sample)
        latent_sample = latent_sample
        reconstruct = x_decoded
        # x_decoded, latent = self(x)
        # loss, recon_loss, kl_loss = self._rec(x_decoded, x.detach(), self.loss_fn)
        is_mss = True
        recon_loss, mi_loss, tc_loss, dw_kl_loss = self._rec2(x_decoded, x, latent_sample, latent_dist, self.n_data, is_mss, self.loss_fn)

        true_samples = torch.randn((latent_mean.shape[0], latent_mean.shape[1])).to(x_decoded.device)
        loss_mmd = compute_mmd(true_samples, latent_sample)
        # loss = recon_loss+mi_loss+tc_loss+dw_kl_loss + loss_mmd
        loss = recon_loss-mi_loss+3*tc_loss+3*dw_kl_loss + loss_mmd

        return loss, recon_loss, dw_kl_loss, x, mi_loss
    ...
    def _train(self, train_loader):
        ...
        for t, X in enumerate(train_loader):
            ...
            loss, recon_loss, kl_loss, _ , mi_loss= self.compute_loss(X)
            loss.backward()
            ...
            epoch_mi += mi_loss.item()
            self.optimizer.step()

            if (t + 1) % self.print_every == 0:
                print('Batch %d, loss = %.4f, recon_loss = %.4f, kl_loss = %.4f, mi_loss = %.4f' % (t + 1, loss.item(),
                                                                                    recon_loss.item(), kl_loss.item(), mi_loss.item()))

        print('Average loss: {:.4f}'.format(epoch_loss / t))
        print('Average mi: {:.4f}'.format(epoch_mi / t))
    ...
    def batch_transform_distribution(self, x):
        """
        Passes the given input tensor into encoder and lambda function
        ...
        :return: intermediate latent vector
        """
        return (self.lmbd.hidden_to_mean(
                    self.encoder(
                        Variable(x.type(self.dtype), requires_grad = False)
                    )
        ),
        self.lmbd.hidden_to_logvar(
                    self.encoder(
                        Variable(x.type(self.dtype), requires_grad=False)
                    )
                ))
    ...
    def transform_compute_q_zCx(self, dataset, save = False):
        """
        ...
        if self.is_fitted:
            with torch.no_grad():
                ...
                for t, x in enumerate(test_loader):
                    x = x[0]
                    x = x.permute(1, 0, 2)
                    samples_zCx_each, params_zCx_each = self.batch_transform_distribution(x)
                    samples_zCx.append(samples_zCx_each)
                    params_zCx.append(params_zCx_each)

                samples_zCx = torch.cat(samples_zCx, 0)
                params_zCx = torch.cat(params_zCx, 0)
                if save:
                    if os.path.exists(self.dload):
                        pass
                    else:
                        os.mkdir(self.dload)
                    samples_zCx.dump(self.dload + '/samples_zCx.pkl')
                    params_zCx.dump(self.dload + '/params_zCx.pkl')
