...
class ModelBase(tf.keras.Model):
    ...
    @property
    def trainable_variables_task_d(self):
        return self._get_trainable_variables(self.task_classifier_d)
    ...
    @property
    def trainable_variables_domain_d(self):
        return self._get_trainable_variables(self.domain_classifier_d)
    ...
    def call_task_classifier_d(self, fe, which_fe=None, which_tc=None,
            which_dc=None, **kwargs):
        if which_tc is not None:
            assert isinstance(self.task_classifier, list)
            return self.task_classifier[which_tc](fe, **kwargs)

        return self.task_classifier_d(fe, **kwargs)
    ...
    def call_domain_classifier_d(self, fe, task, which_fe=None, which_tc=None,
            which_dc=None, **kwargs):
        if which_dc is not None:
            assert isinstance(self.domain_classifier, list)
            return self.domain_classifier_d[which_dc](fe, **kwargs)

        return self.domain_classifier_d(fe, **kwargs)
...
class CodatsModelMakerBase(ModelMakerBase):
    ...
    def make_task_classifier_d(self, num_classes, **kwargs):
        return tf.keras.Sequential([
            tf.keras.layers.Dense(num_classes),
        ])

    def make_domain_classifier_d(self, num_domains, **kwargs):
        return tf.keras.Sequential([
            # Note: alternative is Dense(128, activation="tanh") like used by
            # https://arxiv.org/pdf/1902.09820.pdf They say dropout of 0.7 but
            # I'm not sure if that means 1-0.7 = 0.3 or 0.7 itself.
...
class TimeNetModelMaker(ModelMakerBase):
    ...
    def make_task_classifier(self, num_classes, **kwargs):
        return tf.keras.Sequential([
            self._make_classifier(self.task_layers, num_classes),
        ])

    def make_domain_classifier(self, num_domains, **kwargs):
        return tf.keras.Sequential([
            self._make_classifier(self.domain_layers, num_domains),
        ])
...
class CnnModelBase(ModelBase):
    ...
    def __init__(self, num_classes, num_domains, model_name,
        ...
        self.feature_extractor_s = self._make_single_or_multiple(
            model_maker.make_feature_extractor_s,
            num_feature_extractors, share_most_weights)
        ...
        self.domain_classifier_d = self._make_single_or_multiple(
            lambda **kwargs: model_maker.make_domain_classifier_d(num_domains, **kwargs),
            num_domain_classifiers, share_most_weights)
        self.task_classifier_d = self._make_single_or_multiple(
            lambda **kwargs: model_maker.make_task_classifier_d(num_classes, **kwargs),
            num_task_classifiers, share_most_weights)
...
class MetaFeatureExtractor(tf.keras.Model):
    ...
    def call(self, inputs, labels, **kwargs):
        rnn_output, rnn_state = self.rnn(inputs, labels, **kwargs)
...
class YNDannModelBase:
    ...
    def __init__(self, num_classes, num_domains, global_step,
        ...
        # @property
        # def trainable_variables_fe_s(self):
        #     return self._get_trainable_variables(self.feature_extractor_s)
    ...
    def call_domain_classifier(self, fe, task, **kwargs):
        # Pass FE output through GRL then to DC
        grl_output = self.flip_gradient(fe)
        return super().call_domain_classifier(grl_output, task)

    def call_domain_classifier_d(self, fe, task, **kwargs):
        # Pass FE output through GRL then to DC
        return super().call_domain_classifier_d(fe, task)

    def call_task_classifier_d(self, fe, which_fe=None, which_tc=None,
            which_dc=None, **kwargs):

        grl_output = self.flip_gradient(fe)
        return super().call_task_classifier_d(grl_output)

class YNDannModel(YNDannModelBase, CnnModelBase):
    """ Model with adaptation (i.e. with DANN) """
    # def __init__(self, *args, num_domain_classifiers=None, **kwargs):
    #     # Require that we have num_feature_extractors
    #     assert num_domain_classifiers is not None
    #     super().__init__(*args, num_domain_classifiers=num_domain_classifiers,
    #         **kwargs)
        # self.feature_extractor = self.MetaFeatureExtractor()
    ...
    @property
    def trainable_variables_task_fe_domain_fe_s(self):
        return self.trainable_variables_fe \
            + self.trainable_variables_task \
            + self.trainable_variables_domain \
            + self.trainable_variables_fe_s \
            + self.trainable_variables_domain_d \
            + self.trainable_variables_task_d


    @property
    def trainable_variables_task_fe_domain_fe_d(self):
        return self.trainable_variables_fe \
            + self.trainable_variables_task \
            + self.trainable_variables_domain \
            + self.trainable_variables_domain_d \
    ...
    def call(self, inputs, training=None, **kwargs):
        self.set_learning_phase(training)
        fe = self.call_feature_extractor(inputs, **kwargs)
        # fe2 = self.call_feature_extractor_s(inputs,**kwargs)
        # fe = tf.concat([fe,fe2],1)
        fe_inv = fe[:,:fe.shape[1]//2]
        fe_dep = fe[:,fe.shape[1]//2:]

        task_inv = self.call_task_classifier(fe_inv, **kwargs)
        domain_inv = self.call_domain_classifier(fe_inv, task_inv, **kwargs)

        task_dep = self.call_task_classifier_d(fe_dep, **kwargs)
        domain_dep = self.call_domain_classifier_d(fe_dep, task_dep, **kwargs)
        # return task, domain, fe
        return task_inv, domain_inv, task_dep, domain_dep, fe_inv, fe_dep

    # def call_feature_extractor(self, fe, task, **kwargs):
    #     # Pass FE output through GRL then to DC
    #     grl_output = self.flip_gradient(fe, **kwargs)
    #     return super().call_domain_classifier(grl_output, task, **kwargs)
    ...
    def call_feature_extractor(self, inputs, which_fe=None, which_tc=None,
        ...
        if which_fe is not None:
            assert isinstance(self.feature_extractor, list)
            return self.feature_extractor[which_fe](inputs)

        return self.feature_extractor(inputs)

    def call_feature_extractor_s(self, inputs, which_fe=None, which_tc=None,
            which_dc=None, **kwargs):
        if which_fe is not None:
            assert isinstance(self.feature_extractor, list)
            return self.feature_extractor_s[which_fe](inputs)

        return self.feature_extractor_s(inputs)

    def call_task_classifier(self, fe, which_fe=None, which_tc=None,
            which_dc=None, **kwargs):
        if which_tc is not None:
            assert isinstance(self.task_classifier, list)
            return self.task_classifier[which_tc](fe)
...
@register_model("ynfcn")
class YNFcnModelMaker(CodatsModelMakerBase):
    """
    FCN (fully CNN) -- but domain classifier has additional dense layers
    ...
    def make_feature_extractor_s(self, previous_model=None, **kwargs):
        # # Make a new feature extractor if no previous feature extractor
        # if previous_model is None:
        #     return tf.keras.Sequential([
        #         tf.keras.layers.Conv1D(filters=128, kernel_size=8, padding="same",
...
class YNRnnModel(DannModelBase, RnnModelBase):
    """ Model with adaptation (i.e. with DANN) """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, vrada=True, **kwargs)