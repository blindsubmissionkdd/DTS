    ...
    def _train_step(self, all_data_sources, all_data_target):
        ...
        for i in range(self.ensemble_size):
            ...
            with tf.GradientTape(persistent=True) as tape:
                if self.model_name.startswith('yn') is False:
                    task_y_pred, domain_y_pred, fe_output = self.call_model(
                        x, which_model=i, training=True)
                else:
                    # task_y_pred, domain_y_pred, fe_output = self.call_model(
                    #     x, which_model=i, training=True, task_y_true=task_y_true, domain_y_true=domain_y_true)
                    #  task_inv, domain_inv, task_dep, domain_dep, fe_inv, fe_dep
                    # task_y_pred, domain_y_pred, fe_output = self.call_model(
                    task_inv, domain_inv, task_dep, domain_dep, fe_inv, fe_dep= self.call_model(
                        x, which_model=i, training=True)
                ...
                losses = self.compute_losses(x, task_y_true, domain_y_true,
                    task_inv, domain_inv, task_dep, domain_dep, fe_inv, fe_dep, which_model=i,
                    training=True)
            # Update model
    ...
    def eval_step_list(self, data, is_target):
        ...
        for i in range(self.ensemble_size):
            ...
            if self.model_name.startswith('yn') is False:
                ...
                # task_y_pred, domain_y_pred, fe_output = self.call_model(
                #     x, which_model=i, is_target=is_target, training=False, task_y_true=orig_task_y_true, domain_y_true=orig_domain_y_true)
                task_inv, domain_inv, task_dep, domain_dep, fe_inv, fe_dep = self.call_model(x,
                    which_model=i, is_target=is_target, training=False)
                task_y_pred = task_inv
                domain_y_pred = domain_dep
            # Calculate losses
            ...
            if True:
                # task_inv_r, domain_inv_r, task_dep_r, domain_dep_r = tf.make_ndarray(task_inv), tf.make_ndarray(domain_inv), tf.make_ndarray(task_dep), tf.make_ndarray(domain_dep)
                task_inv_r, domain_inv_r, task_dep_r, domain_dep_r = task_inv.numpy(), domain_inv.numpy(),task_dep.numpy(), domain_dep.numpy()

            losses = self.compute_losses(x, orig_task_y_true, orig_domain_y_true,
                                         task_inv, domain_inv, task_dep, domain_dep, fe_inv, fe_dep,
            ...
            if True:
                np.savez('matwisdm0.npz', task_inv_r=task_inv_r, domain_inv_r=domain_inv_r, task_dep_r=task_dep_r, domain_dep_r=domain_dep_r,task_y_true=task_y_true,domain_y_true=domain_y_true)
...
class MethodDann(MethodBase):
    ...
    def compute_losses(self, x, task_y_true, domain_y_true,
                       task_y_pred, domain_y_pred, fe_output,
...
# with the model architecture and multi-source support = CoDATS
@register_method("ynrnn")
class MethodYN(MethodDann):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.loss_names += ["vrnn"]

    def create_model(self, model_name):
        return models.YNModel(self.num_classes, self.domain_outputs,
            self.global_step, self.total_steps, model_name=model_name)
...
@register_method("yndann")
class YNMethodDann(MethodBase):
    def __init__(self, source_datasets, target_dataset,
            global_step, total_steps, *args, **kwargs):
        self.global_step = global_step  # should be TF variable
        self.total_steps = total_steps
        super().__init__(source_datasets, target_dataset, *args, **kwargs)
        self.loss_names += ["task", "domain", 'task2', 'domain2']
    ...
    def create_optimizers(self):
        ...
        opt["d_opt"] = self.create_optimizer(
            learning_rate=FLAGS.lr*FLAGS.lr_domain_mult)
        opt["t_opt"] = self.create_optimizer(
            learning_rate=FLAGS.lr*FLAGS.lr_domain_mult)
        return opt
    ...
    def compute_gradients(self, tape, losses, which_model):
        total_loss, task_loss, d_loss, task_loss_d, d_loss_d = losses
        grad = tape.gradient(total_loss,
            self.model[which_model].trainable_variables_task_fe_domain_fe_d)
        d_grad = tape.gradient(d_loss,
            self.model[which_model].trainable_variables_domain)
        t_grad = tape.gradient(task_loss_d,
            self.model[which_model].trainable_variables_task_d)
        # s_grad = tape.gradient(task_loss,
        #     self.model[which_model].trainable_variables_fe_s)

        # return [grad, d_grad, s_grad]
        return [grad,d_grad, t_grad]
    def apply_gradients(self, gradients, which_model):
        grad, d_grad,t_grad = gradients
        # grad, d_grad = gradients
        ...
        self.opt[which_model]["d_opt"].apply_gradients(zip(d_grad,
            self.model[which_model].trainable_variables_domain))
        self.opt[which_model]["t_opt"].apply_gradients(zip(t_grad,
            self.model[which_model].trainable_variables_task_d))


@register_method("yndaws")
class MethodYNDaws(YNMethodDann):
    """ Domain adaptation with weak supervision (in this case, target-domain
    label proportions)"""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    #         domain_y_pred, fe_output, which_model, training):
        ...
        nontarget = tf.where(tf.not_equal(domain_y_true, 0))
        task_y_true_nontarget = tf.gather(task_y_true, nontarget)
        task_y_pred_nontarget = tf.gather(task_inv, nontarget)

        task_loss = self.task_loss(task_y_true_nontarget, task_y_pred_nontarget)
        d_loss = self.domain_loss(domain_y_true, domain_inv)

        task_y_pred_nontarget_d = tf.gather(task_dep, nontarget)

        task_loss_d = self.task_loss(task_y_true_nontarget, task_y_pred_nontarget_d)
        d_loss_d = self.domain_loss(domain_y_true, domain_dep)
        ...
        # losses.
        target = tf.where(tf.equal(domain_y_true, 0))
        task_y_pred_target = tf.gather(task_inv, target)
        ...
        # adversarial part converge properly (recall GAN training instability
        # stuff).
        total_loss = task_loss + d_loss + daws_loss + task_loss_d + d_loss_d

        return [total_loss, task_loss, d_loss, task_loss_d, d_loss_d, daws_loss]
    ...
    def create_model(self, model_name):
        return models.YNDannModel(self.num_classes, self.domain_outputs,
            self.global_step, self.total_steps, model_name=model_name)
